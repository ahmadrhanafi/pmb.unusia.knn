...
<th width="1%">:
<th colspan="2"><b><?php echo $id_user;?></b></tr>


<tr>
<td height="24"><label for="nama_user">Nama Pengguna</label>
<td>:<td><input required name="nama_user" class="form-control" type="text" id="nama_user" value="<?php echo $nama_user;?>" size="25" /></td>
</tr>

<tr>
<td height="24"><label for="level">Level</label>
<td>:<td colspan="2">
<input type="radio" name="level" id="level" value="Super Admin" <?php if($level=="Super Admin"){echo"checked";}?>/>Super Admin 
<input type="radio" name="level" id="level"   checked="checked" value="Staf Admin" <?php if($level=="Staf Admin"){echo"checked";}?>/>Staf Admin
</td></tr>
...
<td>:<td><input  required name="username" type="password" class="form-control" id="username" value="<?php echo $username;?>" size="25" />
</td>
</tr>

<tr>
<td height="24"><label for="password">Password</label>
<td>:<td><input  required name="password" type="password" class="form-control" id="password" value="<?php echo $password;?>" size="25" />
</td>
</tr>
...
<td colspan="2"><input name="Simpan" type="submit" id="Simpan" value="Simpan" />
        ...
        <a href="?mnu=id_user"><input name="Batal" type="button" id="Batal" value="Batal" /></a>
</td></tr>
</table>
</form>
<br />
...
$simpan=process($conn,$sql);
	if($simpan) {echo "<script>alert('Data \"$nama_user\" berhasil disimpan !');document.location.href='?mnu=user';</script>";}
		else{echo"<script>alert('Data \"$nama_user\" gagal disimpan...');document.location.href='?mnu=user';</script>";}
	}
	else{
	...
	$ubah=process($conn,$sql);
		if($ubah) {echo "<script>alert('Data \"$nama_user\" berhasil diubah !');document.location.href='?mnu=user';</script>";}
		else{echo"<script>alert('Data \"$nama_user\" gagal diubah...');document.location.href='?mnu=user';</script>";}
	}//else simpan
}
?>
...
$hapus=process($conn,$sql);
	if($hapus) {echo "<script>alert('Data \"$nama_user\" berhasil dihapus !');document.location.href='?mnu=user';</script>";}
	else{echo"<script>alert('Data \"$nama_user\" gagal dihapus...');document.location.href='?mnu=user';</script>";}
}
?>
